...
===================================================
PokerKit gère la mécanique (blinds, dealing, pots, showdown).
DeckSecurity porte le protocole SRA du mental-poker-toolkit
en Python (commit-reveal pour l'intégrité des decks).
"""
...
class DeckSecurity:
    """
    Commit-reveal + SRA verification.
    ...
    def commit_deck(self, hand_round: int, deck: List[str]) -> str:
        ...
        self._commitments[hand_round] = {
            'seed': seed, 'hash': commitment, 'deck_order': list(deck),
        }
        return commitment
...
class PlayerState:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'avatar': self.avatar, 'chips': self.chips, 'stack': self.chips,
            'position': self.position,
            'status': self.status.value if hasattr(self.status, 'value') else str(self.status),
            'current_bet': self.current_bet, 'bet': self.current_bet,
    ...
    def to_pydantic(self) -> TablePlayer:
        return TablePlayer(
            ...
            current_bet=self.current_bet, total_bet=self.total_bet,
...
# ─────────────────────────────────────────────────────────────────────────────
class PokerTable:
    def __init__(self, table_id, name, game_type, max_players,
        ...
        self._deck_security = DeckSecurity()
        self._dealer_btn = 0
        self._hand_round = 0
    ...
    async def add_player(self, user, buy_in: int) -> bool:
        if not self.can_join():
            return False
        ps = PlayerState(user_id=user.id, username=user.username,
    ...
    async def start_game(self):
        ...
        self.game_state = GameState(table_id=self.id, status=GameStatus.IN_PROGRESS,
    ...
    async def _play_hand_pokerkit(self):
        ...
        if len(ordered) == 2:
            ordered[0].is_small_blind = True; ordered[1].is_big_blind = True
        elif len(ordered) >= 3:
            ordered[1].is_small_blind = True; ordered[2].is_big_blind = True
        ...
        if self._ws_manager:
            await self._ws_manager.broadcast_to_table(self.id, {
                'type': 'deck_commitment', 'round': self._hand_round,
        ...
        if reveal and self._ws_manager:
            await self._ws_manager.broadcast_to_table(self.id, {
                'type': 'deck_reveal', 'round': self._hand_round,
                'seed': reveal['seed'], 'deck_order': reveal['deck_order'],
        ...
        self._street = 'preflop'
        await self._broadcast_state()
        logger.info(f"[{self.name}] Hand #{self._hand_round} done — stacks: "
                     f"{[(self.players[u].username, self.players[u].chips) for u in self._pk_uid_map if u in self.players]}")

    def _apply_action(self, pk, actor_idx, action):
        if action is None:
    ...
    async def handle_player_action(self, user_id, action, amount=0):
        await self._action_queue.put(
            PlayerActionRequest(user_id=user_id, table_id=self.id,
    ...
    async def _broadcast_state(self):
        ...
        try:
            ...
            await self._ws_manager.broadcast_to_table(self.id, {'type': 'game_update', 'data': state})
        except Exception as e:
    ...
    def get_state(self) -> dict:
        ...
        return {
            ...
            'min_raise': min_raise, 'max_players': self.max_players,
            'small_blind': self.small_blind, 'big_blind': self.big_blind,
            'betting_round': self._street,
        }

    def get_info(self) -> Table:
        return Table(id=self.id, name=self.name, game_type=self.game_type,
    ...
    def _save_state(self):
        try:
            data = {'table_id': self.id, 'name': self.name,
                    'tournament_id': self.tournament_id,
                    'small_blind': self.small_blind, 'big_blind': self.big_blind,
                    'max_players': self.max_players,
                    'hand_round': self._hand_round, 'dealer_btn': self._dealer_btn,
                    'status': self.status.value if hasattr(self.status, 'value') else str(self.status),
                    'players': {uid: {'user_id': ps.user_id, 'username': ps.username,
    ...
    def load_state(cls, table_id):
        ...
        try:
            ...
            logger.error(f"load_state {table_id}: {e}"); return None
